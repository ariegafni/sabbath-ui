...
export interface HostingRequest {
  ...
  guest_id:
    ...
    | {
      ...
      }; // ID של האורח או אובייקט עם פרטים
  host_id:
    | string
    | {
        _id: string;
        ...
        last_name: string;
        profile_image?: string;
      }; // ID של המארח או אובייקט עם פרטים
  requested_date: string; // תאריך מבוקש
  message: string; // הודעה מהאורח
  status: "pending" | "accepted" | "rejected" | "cancelled";
  response_message?: string; // הודעת תגובה מהמארח
  created_at: string;
  updated_at: string;

  // שדות נוספים למידע על האורח והמארח (לשמירה על תאימות לאחור)
  guest_name?: string;
...
export class HostingRequestService {
  private static baseUrl = createApiUrl("/hosting-requests");
